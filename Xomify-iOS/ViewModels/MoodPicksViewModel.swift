...
final class MoodPicksViewModel {
    ...
    /// Spotify URL of the last playlist we created, used to surface a "Open
    /// in Spotify" affordance once the CTA succeeds. Cleared whenever the
    /// mood changes so stale URLs don't linger.
    var createdPlaylistUrl: URL?
    var isCreatingPlaylist = false
    var createPlaylistError: String?
    ...
    /// Persist the current `tracks` list as a real Spotify playlist on the
    /// signed-in user's account. No-op if the list is empty. Surfaces the
    /// created playlist URL via `createdPlaylistUrl` on success.
    func createPlaylistFromCurrentTracks() async {
        ...
        do {
            let user = try await spotify.getCurrentUser()
            guard let userId = user.id else {
                throw NSError(
                    domain: "MoodPicks",
                    code: 0,
                    userInfo: [NSLocalizedDescriptionKey: "Missing Spotify user id"]
                )
            }
            ...
            let playlist = try await spotify.createPlaylist(
                userId: userId,
                name: "Xomify \(mood.rawValue) — \(dateSuffix)",
                description: "Mood Picks generated by Xomify.",
                isPublic: true
            )

            let uris = tracks.map { $0.uri ?? "spotify:track:\($0.id)" }
            try await spotify.addTracksToPlaylist(playlistId: playlist.id, trackUris: uris)

            if let urlString = playlist.externalUrls?["spotify"], let url = URL(string: urlString) {
                createdPlaylistUrl = url
            } else if let uri = playlist.uri, let url = URL(string: uri) {
                createdPlaylistUrl = url
            }
        } catch {
            createPlaylistError = "Couldn't create playlist: \(error.localizedDescription)"
            print("❌ MoodPicks create playlist failed: \(error)")
        }
